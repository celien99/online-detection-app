            ...
            "cameras": [
                {
                    ...
                    "patchcore_model_path": "./missing.pt",
                }
            ],
            "line_signal": {"enabled": True, "type": "modbus", "host": "127.0.0.1", "port": 502},
            "storage": {"log_dir": str(tmp_path), "screenshot_dir": str(tmp_path)},
...
def test_diagnostics_treats_trigger_alias_as_triggered(tmp_path: Path) -> None:
    model_path = tmp_path / "model.pt"
    model_path.write_bytes(b"model")
    config_path = tmp_path / "config.json"
    _write_config(
            ...
            "cameras": [
                {
                    ...
                    "enabled": True,
                    "patchcore_model_path": "./models/<camera-model>.pt",
                }
            ],
