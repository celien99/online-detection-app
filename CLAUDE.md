...
**`seat_defect_core/`** — Local package providing the ML runtime. The detection pipeline per camera: YOLO segmentation → ROI crop/align → EfficientAD texture anomaly detection → Feature Calibration (normalize/project/whiten) → Filter Classifier (false-positive suppression) → Rule Engine post-processing. Multi-camera fusion combines per-camera results. Entry points: `SeatDefectInspector` class for programmatic use, `python -m seat_defect_core` for CLI.
